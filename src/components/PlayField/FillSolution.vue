...
const cellBorder = computed(() => {
    const res = new Array(state.puzzleLength).fill('border-2 border-transparent cursor-pointer hover:bg-gray-600')
    res[position.value] = 'border-2 border-dashed border-gray-300 cursor-pointer hover:bg-gray-600'
    return res
})
